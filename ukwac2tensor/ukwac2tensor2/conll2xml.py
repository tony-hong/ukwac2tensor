...
# -*- coding: utf-8 -*-
"""
This script uses conll corpus as input for SENNA role labeler. SENNA processes
...
Conversion procedure:
    ...
    5. Write extracted conll sentences into an input file.
...
Performance:
    It takes ~1h to process one conll file on Dual-Core AMD Opteron(R) 2.8Ghz.
...
def detect_missing_files(scan_dir):
    """For unknown reasons conll_converter.py might not process all ~3500 conll
    ...
    conll= set([f.split('.')[2][4:] for f in proc.stdout.read().split()])
    ...
    for m in missed:
        print('qsub -cwd -V -t 1-1:1 -b y ./conll_converter.py -qs -f parallelized_5000_nowiki/conll.fixed-nowiki.node%04d.gz' % int(m))
...
class ConllTools:
    ...
    def clean_bad_chars(mess):
        """
        Remove some conll specific and non-utf8 chars from the file otherwise
    ...
    def insert_sent_delims(sents):
        ...
        Args:
            *sents* (str) -- sentences extracted from conll corpus
    ...
    def get_governors(sent, sent_chunk):
        ...
        Args:
            ...
            | *sent_chunk* (str) -- conll tab separated column data
    ...
    def get_dependants(sent, idx, sent_chunk):
        ...
        Args:
            ...
            |  tuples:
                ``[('first/JJ/3', ('I-A1',)), ('album/NN/4', ('E-A1',))]``
            | *sent_chunk* (str) -- conll tab separated column data
            | *idx* (int) -- index to access correct conll column
...
class SENNA:
    ...
    def run(self, sents, input_fname, output_fname):
        ...
        Args:
            | *out_dir* (str) -- output directory path
            | *sents* (str) -- conll extracted sentences, separated with "\`\`"
...
def extract_conll_data(data):
    """
    Extract 2 columns from conll files, create an ordered dict of
...
def build_xml(lemmas, id_idx, result_fname, malt_data, pverbs):
    ...
    Args:
        ...
        | *result_fname* (str) -- final result file name
        | *malt_data* (defaultdict) -- Malt parser data extracted from conll
...
def conll_convert(args):
    """
    Main function.
    Call methods required for conll corpus conversion into xml representation.
    ...
    try:
        if args.file:
            ...
            name = os.path.join(args.dir,
                                'conll.fixed-nowiki.node' + sge_id + '.gz')
            files = [name]
    except TypeError:
    ...
    # Start conll conversion
    isqsub = args.qsub
    for filename in files:
        conll_fname_path = os.path.join(filename)
        conll_fname = '.'.join([os.path.basename(filename.strip('.gz')),
                                'converted.xml'])
        out_dir = os.path.join(senna.runpath, args.out)
        grid_senna_in = '.'.join([os.path.basename(conll_fname.strip('.gz')),
                                  'input'])
        grid_senna_out = '.'.join([os.path.basename(conll_fname.strip('.gz')),
                                   'output'])
...
if __name__ == '__main__':
    ...
    This script converts conll corpus to xml using SENNA role labeler as a
    sentence parser.
    """)
    ...
    prs.add_argument('-f', '--file',
                     help='Specify conll file to process. If not specified '
                          'default "conll.fixed-nowiki.node{SGE_TASK_ID}.gz" '
                          'is used.',
                     required=False)
    ...
    prs.add_argument('-missing', '--missing',
                     ...
                     'by conll_converter.py to check for any missed files.',
                     required=False)
    args = prs.parse_args()
    if args.missing:
